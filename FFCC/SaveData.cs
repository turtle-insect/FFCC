...
namespace FFCC
{
	class SaveData
	{
		...
		public bool Open(String filename, bool force)
		{
			...
			mBuffer = new Byte[tmp.Length - 20];
			Array.Copy(tmp, mBuffer, mBuffer.Length);
			
			if(force == false)
			{
				Byte[] sum = CalcCheckSum();
				for (int i = 0; i < sum.Length; i++)
				{
					if (tmp[tmp.Length - 20 + i] != sum[i])
					{
						mBuffer = null;
						return false;
					}
		...
		public bool Save()
		{
			...
			Byte[] sum = CalcCheckSum();
			Byte[] output = new Byte[mBuffer.Length + 20];
			Array.Copy(mBuffer, output, mBuffer.Length);
			Array.Copy(sum, 0, output, mBuffer.Length, sum.Length);
		...
		{
			Byte[] check = new Byte[mBuffer.Length - 4];
			Array.Copy(mBuffer, check, check.Length);
			var algorithm = new SHA1CryptoServiceProvider();
			Byte[] result = algorithm.ComputeHash(check);
			algorithm.Clear();
			return result;
		}
